...
local need2Get = 1
local tError = nil
local FP_OBJQOSNAME = "OBJ_VOIPRTP_ID"
local PARA =
{
"DSCPMarkMedia",
...
need2Get = 0
tError = ActionOneOBJ(FP_OBJQOSNAME, PARA, tError, FP_ACTION, FP_IDENTITY)
end
local InstXML1 = ""
if FP_ACTION == "Cancel" then
need2Get = 0
InstXML1, tError = getSpecificInstXML(FP_OBJQOSNAME, FP_IDENTITY, nil, callback, transToFilterTab(t_PARA))
InstXML = InstXML1
end
if need2Get == 1 then
InstXMLTMP, tError = getAllInstXML(FP_OBJQOSNAME, "IGD", tError, nil, transToFilterTab(PARA))
