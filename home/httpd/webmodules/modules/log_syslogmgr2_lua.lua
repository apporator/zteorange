...
function GetLogContent(formerStr)
local logStr = ""
local logLine = ""
local xmlStrTmp = ""
...
local logCache = {}
repeat
logLine = encodeXML(cmapi.show_log(localIP))
...
local formerStr = ""
InstXML = GetLogContent(formerStr)
elseif FP_ACTION == "Refresh" then
need2Get = 0
InstXML = GetLogContent("")
else
end
if need2Get == 1 then
...
xmlStr = xmlStr .. GetLogContent(xmlStr)
InstXML = xmlStr
end
