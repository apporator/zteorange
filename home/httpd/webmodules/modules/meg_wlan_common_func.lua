...
local function MEGQueryWCardAndSSIDIdentity(NICNum)
...
local function MEGQuerySSIDOutputOption(NICNum)
local WLANNICOBJ = "OBJ_WLANSETTING_ID"
local SSIDOBJ = "OBJ_WLANAP_ID"
local WNICIdentityArr = {};
local WLANSSIDTableArr = {};
WNICIdentityArr,WLANSSIDTableArr = MEGQueryWCardAndSSIDIdentity(NICNum)
local OptionStr = ""
...
for i, v in ipairs(WLANBasicIDTable) do
if (env.getenv("CountryCode") == "68" and "2" == session_get(sess_id, "Right") and (i ~= 6 and i ~= 2)) or
(env.getenv("CountryCode") == "68" and "1" == session_get(sess_id, "Right")) or
env.getenv("CountryCode") ~= "68" then
local SSIDName = ""
local instTbl = cmapi.getinst(SSIDOBJ, v)
...
return {
MEGQuerySSIDOutputOption = MEGQuerySSIDOutputOption,
MEGQueryWCardAndSSIDIdentity = MEGQueryWCardAndSSIDIdentity,
}
